...
export default function RSVPSection({ guestName }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (attending) {
      confetti({
        ...
        spread: 70,
        origin: { y: 0.3 },
        colors: ['#F4C2C2', '#E0B4A5', '#FFFDD0', '#D4A574'],
      });
    }
  };
  ...
  if (submitted) {
    return (
      <motion.section
        className="py-24 px-6 bg-card text-center"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-3">
            <motion.button
              ...
              className={`w-full p-4 rounded-2xl border transition-colors ${attending === true ? 'bg-primary border-ring shadow-surface' : 'bg-background border-border'}`}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              <span className={`${fontClass}`}>✅ {t('rsvp.attending')}</span>
            </motion.button>
            <motion.button
              type="button"
              onClick={() => setAttending(false)}
              className={`w-full p-4 rounded-2xl border transition-colors ${attending === false ? 'bg-primary border-ring shadow-surface' : 'bg-background border-border'}`}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              <span className={`${fontClass}`}>😔 {t('rsvp.not_attending')}</span>
            </motion.button>
          </div>
